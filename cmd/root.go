...
func init() {
	...
	RootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.go-osv.yaml)")
	...
	pwd := os.Getenv("PWD")
	RootCmd.PersistentFlags().String("dbpath", filepath.Join(pwd, "go-osv.sqlite3"), "/path/to/sqlite3 or SQL connection string")
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".go-osv" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName(".go-osv")
	}
